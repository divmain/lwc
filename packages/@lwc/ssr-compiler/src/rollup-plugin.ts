...
import { StylesheetConfig, DynamicComponentConfig } from '@lwc/compiler';
import { resolveModule, ModuleRecord } from '@lwc/module-resolver';

import compileScript from './compiler/script.js';
import compileTemplate from './compiler/template.js';
...
const PLUGIN_NAME = 'rollup-plugin-lwc-ssr-compiler';

const IMPLICIT_DEFAULT_HTML_PATH = '@lwc/resources/empty_html.js';
const EMPTY_IMPLICIT_HTML_CONTENT = 'export default function* () {}';
...
export default function lwcSsrCompiler(pluginOptions: RollupLwcOptions = {}): Plugin {
  ...
  return {
    ...
    transform(src, id) {
      ...
      if (extension === '.js') {
        // TODO detect DOM API usage patterns for use in template
        const { code } = compileScript(src, id);
        return { code };
      } else if (extension === '.html') {
        const code = compileTemplate(src, id);
        return { code };
